...
case 1287653383:
case 1825200140:
return this.evaluateFormat (mp, op.intValue, args, tok == 1825200140);
case 134320137:
...
switch (tok) {
case 134217744:
return mp.addXObj (this.getHelixData (bs, 134217744));
case 1073741854:
...
var isAuxiliary = (tok0 == 1275068442);
var pt = 0;
...
var x1 = (args.length < 2 || intValue == 1287653383 ? mp.getX () : null);
...
}if ((pt0 = mp.ptValue (args[0])) == null || tok != 134221842 && args[1].tok == 8) return false;
break;
case 3:
if (tok != 134221842) return false;
if (args[0].tok == 9) {
...
break;
} else if (tok == 134221842 && args[0].tok == 10) {
...
break;
case 4:
if (tok == 134221842) p4 = JU.P4.new4 (JS.SV.fValue (args[1]), JS.SV.fValue (args[2]), JS.SV.fValue (args[3]), JS.SV.fValue (args[0]));
...
Clazz_defineMethod (c$, "evaluateString", 
 function (mp, tok, args) {
var x = mp.getX ();
var sArg = (args.length > 0 ? JS.SV.sValue (args[0]) : tok == 1275068936 ? "" : "\n");
switch (args.length) {
case 0:
case 1:
break;
...
if (x.tok == 7 && tok != 1275068936 && (tok != 1275069447 || args.length == 2)) {
mp.addX (x);
return this.evaluateList (mp, tok, args);
}var s = (tok == 1275069447 && x.tok == 10 || tok == 1275068936 && x.tok == 7 ? null : JS.SV.sValue (x));
switch (tok) {
case 1275069447:
if (x.tok == 10) {
...
bs = this.vwr.getSmilesMatcher ().getSubstructureSet (pattern, this.vwr.ms.at, this.vwr.ms.ac, bsSelected, (tok == 134218760 ? 1 : 2));
} catch (ex) {
if (Clazz_exceptionOf (ex, Exception)) {
this.e.evalError (ex.getMessage (), null);
} else {
...
if (args.length == 2 && pt != null) return mp.addXObj (this.vwr.ms.getSymTemp (false).getSymmetryInfoAtom (this.vwr.ms, bs, xyz, iOp, pt, null, null, 134217744));
...
if (args.length < 1 || args.length > 5) return false;
var i = args.length;
if (i == 1 && args[0].tok == 10) return mp.addX (args[0]);
var distance = 0;
...
return mp.addXObj (this.e.getSmilesExt ().getSmilesMatches (JS.SV.sValue (args[1]), null, bsSelected, null, tok == 134218759 ? 2 : 1, mp.asBitSet, false));
}
...
i = 0;
if (args.length > 2 && JS.SV.sValue (args[2]).equalsIgnoreCase ("unitcell")) tok = 1613369372;
break;
case 4:
var s = JS.SV.sValue (args[1]);
...
return mp.addXStr (this.e.getCmdExt ().dispatch (134221846, true, args));
}, "JS.ScriptMathProcessor,~A");
